<template>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 space-y-6 pb-24">
        <div class="pt-4">
            <h1 class="font-display text-3xl tracking-wider text-white mb-1">{{ $t('leaderboard.title') }}</h1>
            <p class="text-white/30 text-sm">{{ $t('leaderboard.subtitle') }}</p>
        </div>
        ...
        <div v-if="myRank" class="glass-card-accent p-5">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <span class="text-xs text-white/30">{{ myRank.levelName }} · {{ $t('common.lvl') }} {{ myRank.level }}</span>
                </div>
                <div class="text-right shrink-0">
                    <span class="font-mono text-lg text-white">{{ myRank.xp.toLocaleString() }}</span>
                    <span class="text-xs text-white/30 block">{{ $t('common.xp') }}</span>
                </div>
        ...
        <div class="space-y-2">
            ...
            >
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <span class="text-xs text-white/30">{{ player.levelName }} · {{ $t('common.lvl') }} {{ player.level }}</span>
                </div>
        ...
        <div v-if="leaderboard.length === 0 && !loading" class="glass-card p-8 text-center">
            ...
            <p class="text-white/40 text-sm">{{ $t('leaderboard.noPlayers') }}</p>
            <p class="text-white/20 text-xs mt-1">{{ $t('leaderboard.earnXP') }}</p>
        </div>
